...
SIMNOS Docker イメージには以下のセキュリティ対策が含まれています:

- **非ルートユーザー:** コンテナは非ルートユーザー（`nonroot`、UID 456）で実行され、潜在的な脆弱性の影響を限定します。
- **セキュリティパッチ:** イメージはビルド時に `apk upgrade --no-cache` を実行し、ビルド時点で利用可能なセキュリティパッチを適用します。最新のパッチを取得するには、定期的にイメージを再 pull または再ビルドしてください。
