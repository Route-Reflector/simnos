...
| return a dict (`{"output", "new_mode", "exit"}`) from a custom command handler | Return `str` (or `None`) only, and author the transition statically on the command (`new_prompt`, or A3 `new_mode` / `exit` / `transitions`) — a dict return now answers `% Internal error`. See #317 |
