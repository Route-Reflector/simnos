...
- Add terminal paging (`--More--`) for long command output (#307). The page height follows the PTY / Telnet NAWS rows (falling back to `sys_config.paging.default_rows`, default 24); a `terminal length 0`-class command disables paging for the session (the `disables_paging` data flag — the primary scraper defence, since automation clients send one during session preparation), and in addition a line-count gate sends output that fits the page height (e.g. netmiko's `height=1000`) straight to the plain response, preserving byte parity. The `--More--` string is platform data (`platform.yaml` `paging.more_prompt`, Cisco default `" --More-- "`)
