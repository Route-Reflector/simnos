...
- **サーバープラグイン:** 接続先の各種サーバーを実行するプラグインです。SSH（`paramiko` 経由）と Telnet（raw socket）をサポートしています。
...
- **エコーコアレッシング:** サーバープラグインがエコー文字とシェルレスポンスを単一のネットワークパケットにバッチ処理し、netmiko `send_command()` での断続的な空出力を防止します。
