...
- **サーバープラグイン:** 接続先の各種サーバーを実行するプラグインです。SSH（`asyncssh` 経由）と Telnet（`telnetlib3` 経由）をサポートしています。
...
- **スレッドセーフ I/O:** `TapIO` ブリッジは条件変数によるウェイクアップ付きブロッキング `readline()` をサーバーとシェル層間で提供していました（v3 で撤去 — asyncssh / telnetlib3 上の非同期 push-dispatch セッションループに置き換え、#297）。
