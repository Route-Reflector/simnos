...
- **Server Plugins:** plugins responsible for running various servers to connect with. It supports SSH (via `paramiko`) and Telnet (raw socket).
...
- **Echo coalescing:** Server plugins batch echo characters and shell responses into single network packets, preventing intermittent empty output with netmiko `send_command()`.
