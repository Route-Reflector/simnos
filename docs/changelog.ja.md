...
- `BaseDevice` 基底クラスを `simnos/plugins/nos/base_device.py` に移設 (#350)。従来は `simnos/plugins/nos/platforms_py/_templates/base_template.py` に「authoring テンプレート」を名乗って置かれていましたが、実際は全 device モジュール (と core) が import する必須のランタイム基底クラスでした。旧 import path と空になった `_templates/` サブパッケージは削除。外部の py handler モジュールは import 行1 箇所の更新のみが必要で、クラス自体は不変です
